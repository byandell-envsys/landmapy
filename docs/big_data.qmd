...
from landmapyr.cdcplaces import shp_tract_path, download_census_tract, download_cdc_disease, join_tract_cdc
from landmapyr.naip import naip_path, download_naip_scenes, ndvi_naip_df, merge_ndvi_cdc
from landmapyr.explore import var_trans, train_test
from landmapyr.plots import plot_gdfs_map, plot_matrix, plot_train_test, plot_tract_gdf
```

Landmapry routines with hvplot
...
```{python}
#| label: fig-chicagotracts
#| fig-cap: "Chicago Census Tracts"
plot_tract_gdf(chi_tract_gdf)
```
...
### GeoViews Dynamic Map

```{python}
#| eval: true
model_fit = gvplot_ndvi_index(ndvi_cdc_gdf)
resid = gvplot_resid(logndvi_cdc_gdf)
