...
```{python}
#| eval: false
pip install -q plotnine scikit-misc
```
...
from landmapyr.ggplot import ggplot_xy # plot with ggplot (plotnine)
```
...
Include a citation of the MACAv2 data.
...
Python package [plotnine](https://plotnine.readthedocs.io/)
uses the same
...
as R's [ggplot2](https://ggplot2.tidyverse.org/).
Here we demonstrate its use for the climate project.
...
While images are typically reprojected to a higher resolution,
associations among measurements should 
be assessed on the lower resolution
to avoid artificially inflating sample sizes and reduce spatial autocorrelation.
...
```{python}
#| fig-title: "ggplot now 45 vs 85"
ggplot_xy(hab_df, 'now45', 'now85', identity=True)
```

```{python}
#| fig-title: "ggplot now 45 vs fut 45"
ggplot_xy(hab_df, 'now45', 'fut45', identity=True)
```

```{python}
#| fig-title: "ggplot sand vs slope"
ggplot_xy(hab_df, 'sand', 'slope', log = ['x','y'])
```

```{python}
#| fig-title: "ggplot now 45 vs slope"
...
Pivot (melt) data to longer format to make time series plots.
...
```{python}
#| fig-title: "ggplot now 45 time with loess"
...
```{python}
#| fig-title: "ggplot time with loess curve"
